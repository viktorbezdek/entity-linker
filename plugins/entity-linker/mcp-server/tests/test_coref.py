...
def test_propagate_same_surface_inherits_entity() -> None:
    # Two "Viktor" spans: first auto-linked, second ambiguous → second inherits
    r1 = _res("Viktor", "viktor-bezdek", 0.95, span_start=0)
    r2 = _res("Viktor", "viktor-bezdek", 0.75, span_start=20)
    out, warnings = propagate([r1, r2])
    assert all(r.entity_id == "viktor-bezdek" for r in out)
    assert len(warnings) == 0


def test_propagate_conflict_emits_drift_warning() -> None:
    # Same surface, different entity IDs → entity_drift warning
    r1 = _res("Viktor", "viktor-bezdek", 0.92, span_start=0)
    r2 = _res("Viktor", "viktor-novak", 0.80, span_start=20)
    out, warnings = propagate([r1, r2])
...
def test_propagate_different_surfaces_unchanged() -> None:
    r1 = _res("Viktor", "viktor-bezdek", 0.90, span_start=0)
    r2 = _res("Tomas", "tomas-novak", 0.88, span_start=20)
    out, warnings = propagate([r1, r2])
    assert out[0].entity_id == "viktor-bezdek"
    assert out[1].entity_id == "tomas-novak"
    assert len(warnings) == 0
...
def test_propagate_single_span_unchanged() -> None:
    r = _res("Viktor", "viktor-bezdek", 0.91)
