...
def test_to_markdown_golden() -> None:
    ...
    assert result == "Hello [Stefan](@person:stefan-weber) world"


def test_to_markdown_user_confirmed_gets_question_mark() -> None:
    res = Resolution(
        surface="Stefan", span_start=6, span_end=12,
        entity_id="stefan-weber", entity_type="person",
...
def test_to_xml_golden() -> None:
    ...
    assert 'id="stefan-weber"' in result
...
def test_to_sidecar_has_correct_offsets() -> None:
    ...
    assert r["start"] == 6
    assert r["end"] == 12
    assert r["entity_id"] == "stefan-weber"
